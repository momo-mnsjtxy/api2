...
function email($ArrEmail,$AddEmail,$EmailName,$EmailTitle,$EmailBody,$File,$FileName){
    ...
    $mail->Username = 'info@gqink.cn'; // 邮箱用户名
    $mail->Password = 'Me@42674264';//邮箱密码
    $mail->SMTPSecure = 'ssl';//加密方式
    $mail->Port = 465; //端口

    //接收人
    $mail->setFrom('info@gqink.cn', '与梦城');
    $mail->addReplyTo('i@gqink.cn', '与梦城'); //回复地址
    $mail->addAddress($AddEmail,$EmailName);     // 添加一个收件人
    //$mail->addAddress($addemail);               // 可以只传邮箱地
    if(is_array($ArrEmail)){
        foreach($ArrEmail as $v) {
            $mail->addAddress($v);               // 添加多邮箱
        }
    }
    ...
    if($File != "" && $FileName != ""){
        $mail->addAttachment($File,$FileName);    // 可设置名字
    }
...
//email($ArrEmail,$AddEmail,$EmailName,$EmailTitle,$EmailBody,$File,$FileName);
//     数组传输, 一个邮箱 , 邮箱名字 , 邮件标题 , 邮件内容 , 文件地址 , 文件名字 
 ...
 */
    ...
    QRcode::png($text,false,QR_ECLEVEL_L,10,1);
}
...
//汉字转拼音调用函数
function pinyin($test){
	$text = hanzi();
	$r = substr(substr($text,strpos($text,$test)),0,strpos(substr($text,strpos($text,$test)), ','));
	return str_replace($test,"",$r);
}
